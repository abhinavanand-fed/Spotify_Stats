    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        response.items.forEach((item, i) => {
          ...
          resultsHtml +=
            '<div class="column wide artist item"><a href="' +
            url +
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.items.length === 0) {
          ...
          response.items.forEach((item, i) => {
            ...
            resultsHtml +=
              '<div class="column wide track item"><a href="' +
              url +
              '" target="_blank"><img src="' +
              image +
              '"></a><h4>' +
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        if (jqXHR.status === 401) {
          ...
          $("#track-button").removeClass("loading");
          $("#results").html(
            "<p>Error retrieving top tracks. Please try again later.</p>"
          );
        }
      },
    });
...
function ifError(error) {
  ...
  switch (error) {
    ...
    default:
      errorMessage =
        "Unable to authorize through Spotify Web API. Please try logging in again.";
  }
  alert(errorMessage);
}
